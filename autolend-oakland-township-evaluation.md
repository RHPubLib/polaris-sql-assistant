...
| 16 | Staff SSO via Google OAuth/OIDC or SAML + FIDO2 MFA; named accounts vs. shared | Bill (vendor) | **PARTIAL 6/18** — Bill answered the *on-device* admin (local card) + cited his Academic OAuth; did **not** confirm native Google OIDC/SAML, FIDO2, or named-vs-shared on the staff *web* UI. We solve it via our own proxy (Model 2); native IdP/FIDO2 + Academic-OAuth details remain unconfirmed |
